...
if (isset($_GET['del'])) {
    $id = $_GET['del'];
    mysqli_query($con, "DELETE FROM basses WHERE id=$id");
...
<div class="container-md" style="height: auto; background-color: navajowhite; border-style: groove; margin-top: 5.5%">
    <h5 style="margin-top: 2%"> <a class="test33" href="bassesdb_create.php"> Create New </a> </h5>

    <form class="form-inline my-2 my-lg-0" method="POST" action="test_search_b.php" style="padding-top: 1%">
    ...
    <form class="form-inline" action="bassesdb.php" style="margin-top: 1%">
        <button type="submit" class="btn btn-info">Show All</button>
    </form>


    <?php
    $result = mysqli_query($con,"SELECT * FROM basses");

    if(isset($_SESSION['searchid'])){
        $varid = $_SESSION['searchid'];
        $result = mysqli_query($con,"SELECT * FROM basses WHERE id LIKE '%$varid%'");
        unset($_SESSION['searchid']);
    } elseif (isset($_SESSION['searchmodel'])) {
        $varm = $_SESSION['searchmodel'];
        $result = mysqli_query($con,"SELECT * FROM basses WHERE model LIKE '%$varm%'");
    ...
    <div class="table-responsive-lg" style="height: auto;margin-top: 1%; color: black">
        <table class="table table-bordered table-active">
            ...
            <tr>
                ...
                <td> <a href="updateb.php?id=<?php echo $row['id']; ?>" style="color:green">Edit</a> </td>
                <td><a href="bassesdb.php?del=<?php echo $row['id']; ?>" style="padding-left: 5%;color: red">Delete</a> </td>
            </tr>
            </tbody>
            <?php
